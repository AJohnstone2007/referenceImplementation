...
Set<Parent> jointForest = new HashSet<Parent>();
...
boolean useTangentInterpolator = true;
...
float[] prevOutTan = new float[3];
float[] curOutTan = new float[3];
float[] curInTan = new float[3];
...
kvPrev = ((MFloat) kPrev.getData("kv")).get();
...
kvNext = ((MFloat) kNext.getData("kv")).get();
...
int prevOutTanType = tan.get();
int curInTanType = tan.get();
int curOutTanType = tan.get();
...
double timeDeltaPrev = (durationPrev / FPS) * 1000f / 3.0f;
double timeDeltaNext = (durationNext / FPS) * 1000f / 3.0f;
if (true) {
}
...
KeyValue keyValue = new KeyValue(property, kv, interp);
