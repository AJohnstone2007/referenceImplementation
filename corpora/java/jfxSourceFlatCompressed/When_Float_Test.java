...
public class When_Float_Test extends WhenTestBase<Number, Property<Number>> {
@SuppressWarnings("unchecked")
public When_Float_Test() {
super(
-(float) Math.PI, 0.0f, EPSILON_FLOAT, Float.MAX_VALUE,
new SimpleFloatProperty(), new SimpleDoubleProperty(), new SimpleFloatProperty(), new SimpleLongProperty(), new SimpleIntegerProperty()
);
}
@SuppressWarnings("unchecked")
@Override
...
return new Binding[] {
Bindings.when(cond).then(op0.floatValue()).otherwise(op1.doubleValue()),
Bindings.when(cond).then(op0.floatValue()).otherwise(op1.floatValue()),
Bindings.when(cond).then(op0.floatValue()).otherwise(op1.longValue()),
Bindings.when(cond).then(op0.floatValue()).otherwise(op1.intValue())
};
}
@Override
public void check(Number expected, Binding<Number> binding) {
assertEquals(expected.floatValue(), binding.getValue().floatValue(), EPSILON_FLOAT);
}
}
