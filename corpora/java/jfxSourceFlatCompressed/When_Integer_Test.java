...
public class When_Integer_Test extends WhenTestBase<Number, Property<Number>> {
@SuppressWarnings("unchecked")
public When_Integer_Test() {
super(
Integer.MIN_VALUE, 0, Integer.MAX_VALUE, 1,
new SimpleIntegerProperty(), new SimpleDoubleProperty(), new SimpleFloatProperty(), new SimpleLongProperty(), new SimpleIntegerProperty()
);
}
@SuppressWarnings("unchecked")
@Override
...
return new Binding[] {
Bindings.when(cond).then(op0.intValue()).otherwise(op1.doubleValue()),
Bindings.when(cond).then(op0.intValue()).otherwise(op1.floatValue()),
Bindings.when(cond).then(op0.intValue()).otherwise(op1.longValue()),
Bindings.when(cond).then(op0.intValue()).otherwise(op1.intValue())
};
}
@Override
public void check(Number expected, Binding<Number> binding) {
assertEquals(expected.intValue(), binding.getValue().intValue());
}
}
