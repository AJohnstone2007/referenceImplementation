...
functor Simple (structure SimpleLib : SIMPLELIB): SIMPLE =
struct
open SimpleLib
...
fun reflect dir (node:int*int) (A:real array2) = sub2(A, dir node)
...
fun sum_this i =
let fun sum_this' j = f(i,j)
in sum_list(map sum_this' (from(0,degree)))
end
in
sum_list (map sum_this (from (0,degree)))
end
...
fun ohandle zone = raise Overflow
in for_interior_zones(fn zone => (update2(M,zone, interior_sigma zone)));
M
end
...
in
ignore (int_zones (fn (k,l) => update2(theta, (k,l), interior_theta (k,l))));
theta
end
...
fun runit step_count =
let fun iter (i,state) = if i = 0 then state
else iter(i-1, compute_next_state state)
in iter(step_count, compute_initial_state())
end
fun simple () =
ignore (runit iterations)
end
;
