...
of {miscStruct : string,
tableStruct : string,
tokenStruct : string,
actionsStruct : string,
valueStruct: string,
ecStruct : string,
arg: string,
tokenSig : string,
...
wasError : unit -> bool) =>
let
val (user_requires,header) = stripRequires header
...
of SOME i => i
| _ => raise General.Fail "Impossible",name)
end
...
case SymbolHash.find(symbolName symbol,symbolHash)
of SOME i => i
| _ => raise General.Fail "Impossible",ty)
...
val entries = ref 0
in let val {dir,file} = OS.Path.splitDirFile spec
val file = implode (map (fn c => if c= #"." then #"_" else c)
(explode file))
val result = TextIO.openOut(OS.Path.joinDirFile
{dir=dir,
file="_"^ file ^ ".sml"})
val sigs = TextIO.openOut (OS.Path.joinDirFile {dir=dir,
file=file ^ ".sml"})
val pos = ref 0
...
in
sayln ("require \""^file^"\";");
sayln "require \"$.lib.base\";";
sayln "require \"$.basis.__char\";";
sayln "require \"$.basis.__string\";";
sayln "require \"$.basis.__array\";";
sayln user_requires;
case header_decl
