functor HuetFUN (structure T : TERM
...
struct
val CompName = "Huet's Left-Linear Completion"
type EqualitySet = Es.EqualitySet
type State = State.State
open T T.Sig En Es Eq iEq Str Sp Ord R C Ct State
fun complete S step (E,R) H =
let
val (A,T,ENV,AC) = (get_Signature S, get_Parser S, get_Environment S, get_EqTheory S)
...
fun cps R1 e R2 E = (write_terminal "Generating Critical Pairs\n";
merge_eqsets (Strategy A)
(cpall A (Strategy A) R1 e R2 E)
(foldl_over_equations (cpg A (Strategy A) R1 "AC-eq" e) E AC)
)
val normN = normalisebyNew A (Strategy A)
fun delay s (A,T,env) newenv H rE R e1 =
(printequality A s e1; newline () ;
write_terminal "Do you wish to try Splitting by Introducing new Operator? (y/n) ";
let val reply = hd (explode (read_line std_in))
in if reply = "y" orelse reply = "Y"
...
else (write_terminal "Do you wish to continue? (y/n) ";
let val reply = hd (explode (read_line std_in))
in if reply = "y" orelse reply = "Y"
...
val E' = ins_by_strat (normN newrule rE [R'']) E'
val (b,H') = consider_conjectures A E' [R''] H
in if b
then ((E',R'',H'), (A,T,newenv))
else let val E'' = (cps R'' newrule R'' E')
val d = write_terminal "Finished generating Critical Pairs.\n"
in if stop step A [E'',R''] H'
then ((E'',R'',H'), (A,T,newenv))
else kb (A,T,newenv) H' E'' R''
end
end
else delay "Non-Left-Linear Rule: " (A,T,env) newenv H rE R newrule
)
else delay "Unorientable Equation By Term Ordering Conflict with Sort Ordering : \n " (A,T,env) newenv H rE R e1
end)
handle Interrupt => (write_terminal "User Interrupts Huet's Left-Linear Completion. \n";
message_and_wait () ;
((E,R,H),(A,T,env)))
end
