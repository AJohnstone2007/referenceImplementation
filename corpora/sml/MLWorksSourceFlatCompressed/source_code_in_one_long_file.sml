...
|TLet of (string * EXP) list * string * string list
|Elem of string * EXP
...
fun map_union f [] = []
| map_union f (x::l) = union (f x,map_union f l) ;
fun snd (_, y) = y;
...
fun rest [] I = []
| rest ((x,Var y)::l) I = if x = y then rest l I
...
else let val newvar = gensym I v
val I' = newvar::I
in increase_context (inst I' [(v,Var newvar)] (Lam(vl,e)))
(inst_type I' [(x,Var newvar)] (Prod(argl,t)))
I' ((newvar,A)::C)
...
| get_defs (IDef(Impl _,def,_,_,_)::defs) = [([],def)]
| get_defs [] = []
...
| Impl(c,l) => try_match_pattern (Impl(c,l)) [] I env
...
| Lam(vb,e) => HNform (Lam(vb,e))
| e => raise simplereduce_result_error e
end
and try_match_pattern (f as Impl(c,l)) argl I env =
...
;
fun check_conv e1 e2 (t as Prod(_,_)) env C I =
if e1 = e2 then Ok []
...
in case e of
(Var x) => if isqsym x then Elem ("Set",e)
...
New_env
| New_scratch
| Include_file of string
| Reinclude_file of string
| Load_file of string
| Reload_file of string
| Load_scratch of string
| Save_defs of string
| Save_scratch of string
| Move_to_env of string list
| New_exp of string
| New_type of string
| New_context of string
| New_subst of string * CONDEF_SUBST
| New_set of string
| New_constr of string * string
| New_impl of string
| Move_to_scratch of string list
| Extend_context of string * string
| Change_context of string * CONDEF_CONTEXT
| Abstr of WINDOW_PATH
...
else raise Subst_on_non_subst_form f
...
in case ftype of
(Prod(argt,t')) =>
if (len argl) > (len argt) then ET_Noteq (App(f,argl),t)
...
let val newc = create "" ((c::I) @ global_names env) c
...
if c = x then
let val c' = create "" (I @ global_names env) c
...
| Tunfold_in_wdef I c T (WExpr e) = WExpr e
...
let val (newinfo,newdef) = mark_new_abbr def info isnew true
...
let val (newinfo,newdef) = mark_new_constr def info isnew name true
...
else let val (newinfo,newdef) = mark_new_prim d info isnew true
in Env(newdef::prims,abbrs,newinfo,scr1,scr2)
end
end
| add_prim_to_env def isnew (env as Env(prims,abbrs,info,scr1,scr2)) =
let val (newinfo,newdef) = mark_new_prim def info isnew true
in Env(newdef::prims,abbrs,newinfo,scr1,scr2)
end
;
...
let val (newdefs,newenv) = load_file true filename env
...
let val (newdefs,newenv) = reload_file true filename env
in (show_defs newdefs;if undo flag then (flag,coml,newenv,UNDO env) else (flag,coml,newenv,oldenv))
end
handle error => (errormessage error ; (flag,coml,env,oldenv))
;
fun do_include_file filename (flag,coml,env,oldenv) =
let val (newdefs,newenv) = load_file false filename env
in (show_defs newdefs;if undo flag then (flag,coml,newenv,UNDO env) else (flag,coml,newenv,oldenv))
end
handle error => (errormessage error ; (flag,coml,env,oldenv))
;
fun do_reinclude_file filename (flag,coml,env,oldenv) =
let val (newdefs,newenv) = reload_file false filename env
in (show_defs newdefs;if undo flag then (flag,coml,newenv,UNDO env) else (flag,coml,newenv,oldenv))
end
handle error => (errormessage error ; (flag,coml,env,oldenv))
;
...
and print_wdefinition pattname (WUnknown) = "?"
...
| _ => raise Path_error (print_wpath (WP(c,p))))
| get_abbr_info (SDef _) p env = CInfo(empty_context)
;
...
[Ppath p1] => get_info_from_context DefC p1 env
| _ => raise Path_error (print_wpath (WP(c,p))))
| get_u_abbr_info (USDef _) p env = CInfo(empty_context)
;
...
create qsym (newnames @ I @ (global_names env)) "T"
;
...
let val ftype = get_real_type f DefC I env
val name_list = create_names qsym (n+len argl) ftype
((get_subset_fidents [] e) @ I @ (global_names env))
...
| _ => (env,App(f,argl),newnames,ET_Noteq (App(f,argl),t))
end
...
if isunknown c env then
let val (t,DefC) = get_massaged_type_and_context c (t,DefC) env
...
handle error => (flag,coml,env,oldenv)
;
...
| print_history filename (Env(p,a,Info(x,y,z,_::historylist),s,b)) =
let val outstr = open_out filename
