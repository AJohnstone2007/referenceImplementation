...
import uk.ac.rhul.cs.csle.art.cfg.referenceFamily.gll.GLLBL;
import uk.ac.rhul.cs.csle.art.cfg.referenceFamily.gll.GLLHP;
...
public class Reference {
  ...
  private ReferenceParser parser = new GLLHP();
    ...
    for (String a : args) {
      ...
      if (parts.length == 2) {
        if (parts[1].equals("art"))
          ...
          parseString(testString, true, grammar, parser, lexer, true);
        }
      } else
        switch (a) {
        case "!builtinTests":
  ...
  private void parseString(String inputString, boolean outcome, Grammar grammar, ReferenceParser parser, LexerLM lexer, boolean suppressEcho) {
