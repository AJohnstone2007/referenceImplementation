...
public class ARTV4OLD {
  private final OLDPipelineParamaters pp = new OLDPipelineParamaters(new ITermsLowLevelAPI());

  private OLDModule currentModule;

  public ARTV4OLD(String specification) {
    ...
    for (Integer m : pp.modules.keySet()) {
      OLDModule module = pp.modules.get(m);
    ...
    new OLDPipeline(pp).interpretDynamicDirectives(pp.mainModule);
  }

  OLDModule findModule(int moduleTerm) {
    OLDModule ret = pp.modules.get(moduleTerm);
    if (ret != null)
      throw new ARTUncheckedException("Attempt to redefine " + (moduleTerm == 0 ? " unnamed module" : " module " + pp.iTerms.toString(moduleTerm)));
    pp.modules.put(moduleTerm, (ret = new OLDModule(pp, moduleTerm))); // set up default main module
    return ret;
  }
