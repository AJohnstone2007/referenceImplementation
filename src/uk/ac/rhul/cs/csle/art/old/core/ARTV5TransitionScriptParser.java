...
import uk.ac.rhul.cs.csle.art.old.cfg.referenceFamily.Reference;
import uk.ac.rhul.cs.csle.art.old.cfg.referenceFamily.ReferenceParser;
import uk.ac.rhul.cs.csle.art.old.cfg.referenceFamily.grammar.GIFTKind;
import uk.ac.rhul.cs.csle.art.old.cfg.referenceFamily.grammar.GKind;
...
public class ARTV5TransitionScriptParser extends ReferenceParser {
  ...
  public void parse() {
    ...
    if (!accepted) parserError("GLLBL ");
  }

  private void parserError(String msg) {
    System.err.print(Reference.echo(msg + "syntax error", positions[sppfWidestIndex()], inputString));
  }
...
private SPPFPN firstSelectedPackNode(SPPFN sppfn) {
  ...
  for (SPPFPN p : sppfn.packNS)
    ...
    if (candidate == null)
      ...
      System.err.println("Multiple selected pack nodes at SPPF node " + sppfn);
  if (candidate == null) System.err.println("No selected pack nodes found at SPPF node " + sppfn);
  return candidate;
}
