...
public class ARTEarleyLinkedAPI extends ARTParserBase {
  /*
  ...
  public ARTEarleyLinkedAPI(ARTGrammar grammar) {
    super(grammar);
        ...
        if (c instanceof ARTGrammarInstanceNonterminal) {
          ...
          for (ARTGrammarInstanceCat p : ((ARTGrammarElementNonterminal) c.getPayload()).getProductions()) {
            ...
            if (inSigmaN(deltaSlot)) {
              if (!eSets.get(i).contains(eSetCandidate1)) {
          ...
          if ((v = hMap.get(c.getPayload())) != null && betaSlot.getGuard().contains(input.get(i + 1))) {
            ...
            if (inSigmaN(betaSlot)) {
              if (!eSets.get(i).contains(eSetCandidate2)) {
          ...
          for (ARTEarleyItem e : new LinkedList<>(eSets.get(h))) {
            ...
            if (e.slot.getSibling() != null && e.slot.getSibling().getPayload() != null && e.slot.getSibling().getPayload().equals(D)) {
              ...
              if (inSigmaN(deltaPreSlot)) {
                if (!eSets.get(i).contains(candidateItem)) {
